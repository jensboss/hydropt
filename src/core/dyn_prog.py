...
def backward_induction(n_steps, volume, num_states, actions, net_actions,
    ...
    # allocate momory
    rewards_to_evaluate = np.zeros((actions.shape[0], num_states_tot))
    ...
    for k in np.arange(n_steps):
        ...
        for act_index, (action, net_action) in enumerate(zip(actions, net_actions)):
            L = transition_matrix(volume, num_states, net_action-inflow_now)
            # print(k, act_index , '\n', L.todense()[:,0])
            immediate_reward = np.sum(action*hpfc_now)
...
def forward_propagation(n_steps, volume, num_states, basins_contents, actions, net_actions,
                        inflow, action_grid):
    net_actions_taken = np.zeros((n_steps, net_actions.shape[1]))
    actions_taken = np.zeros((n_steps, actions.shape[1]))
    ...
    for k in np.arange(n_steps):
        ...
        net_actions_taken[k] = net_actions[action_grid[k, state_index]]
        actions_taken[k] = actions[action_grid[k, state_index]]
        vol[k+1, :] = vol[k,:] - net_actions_taken[k, :] + inflow[k, :]
        
    return actions_taken, net_actions_taken, vol